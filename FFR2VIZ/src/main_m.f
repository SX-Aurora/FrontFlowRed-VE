...
!
            write(*,*) ' #### Writing FLUENT Header...'
            call writeFLUENTHeader(outFVfn,25,gridOnly,rtrnStat)
!
            if(rtrnStat/=0) then
               write(*,*) 'ABEND: Cannot create output file.'
               stop
            end if
            write(*,*)' #### FLUENT Header completed.'
!
            call append_resultfluent(outFVfn,25,gridOnly,rtrnStat)
            if (rtrnStat/=0) then
               write(*,*) 'ABEND: Cannot create output file'
               stop
            endif
            write(*,*)' #### Appending data for FLUENT completed.'
!#endif
!
!#ifdef SCRYU
!        --- SCRYU --------------
...
!
            write(*,*) ' #### Writing SCRYU/Tetra Header...'
            call writeSCRYUHeader(outFVfn,25,rtrnStat)
            if(rtrnStat/=0) then
               write(*,*) 'ABEND: Cannot create output file.'
               stop
            end if
            write(*,*)' #### SCRYU/Tetra Header completed.'
!
            call append_resultscry(outFVfn,25,gridOnly,rtrnStat)
            if (rtrnStat/=0) then
               write(*,*) 'ABEND: Cannot create output file'
               stop
            endif
            write(*,*)' #### Appending data for SCRYU/Tetra completed.'
!#endif
!
!#ifdef STARCD
!        --- PROSTAR(user subroutine file) --------------
         else if (resultFormat == 'SC') then
!
            write(*,*) ' #### Writing STAR-CD(user post) data ...'
            call append_resultstar(outFVfn,25,
     &           gridOnly,gridFormat,rtrnStat)
            if (rtrnStat/=0) then
               write(*,*) 'ABEND: Cannot create output file'
               stop
            endif
            write(*,*)' #### writing data for STAR-CD(user post data)',
     &           ' completed.'
!#endif
!#ifdef ENSIGHT
!        --- ENSIGHT --------------
...
!
            write(*,*) ' #### Writing EnSight Header...'
            call writeENSIGHTHeader(outFVfn,25,gridOnly,
...
!
            ...
            write(*,*)' #### EnSight Header completed.'
!
            call append_resultensight(outFVfn,25,MPprocNum,gridOnly,
     &                                animeStat,animeExt,animeStep,
     &                                rtrnStat)
            if (rtrnStat/=0) then
               write(*,*) 'ABEND: Cannot create output file'
            ...
            write(*,*)' #### Appending data for EnSight completed.'
!#endif
         end if
!
! ------------------------------------------
